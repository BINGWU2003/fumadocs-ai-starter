...
function rehypeWrapWords() {
  return (tree: Root) => {
    visit(tree, ['text', 'element'], (node, index, parent) => {
      ...
      const nodes: ElementContent[] = words.flatMap((word) => {
...
function Pre(props: ComponentProps<'pre'>) {
  ...
  return <DynamicCodeBlock lang={language} code={content.trimEnd()} />;
}
...
function createProcessor(): Processor {
  ...
  return {
    async process(content) {
      const markdown = processor.parse({ value: content });
      const hast = await processor.run(markdown);
